...
若我们使用的是 Vertx 默认的 C3P0（`DataSourceProvider`），可以使用下面的配置参数：
...
如果我们的 Vertx 应用程序打包成 *fat jar*，要确保 jdbc 驱动包含在里面。如果我们的 Vertx 应用程序通过 `vertx`命令行启动，要确保 JDBC 驱动包在`${VERTX_HOME}/lib`路径里面。
