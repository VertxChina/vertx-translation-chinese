...
Vert.x MySQL / PostgreSQL Client （以下简称客户端）基于 Mauricio Linhares 写的异步驱动  [async driver](https://github.com/mauricio/postgresql-async) ，让 Vert.x 应用能够以异步无阻塞的方式访问 MySQL 或者 PostgreSQL 数据库。

## 使用 MySQL and PostgreSQL 客户端

这部分内容阐述了如何在您的应用中配置 MySQL / PostgreSQL 客户端。

### 在常规应用中

要使用 MySQL / PostgreSQL 客户端，您需要把下面的 jar 包加入 `CLASSPATH`:
...
或者，您可以编辑位于  `$VERTX_HOME` 路径下的文件  `vertx-stack.json`，并且设置 `vertx-mysql-postgresql-client` 的依赖为 `"included": true`。完成后，执行命令：`vertx resolve --dir=lib --stack= ./vertx-stack.json`，就会下载此客户端及它的依赖。
...
大部分情况下，您都将希望不同的 client 对象共享一个连接池。
...
您还可以像下面这样，在创建一个 客户端对象的时候指定数据源的名称：
...
每次调用此方法，就相当于在调用  [`MySQLClient.createShared`](http://vertx.io/docs/apidocs/io/vertx/ext/asyncsql/MySQLClient.html#createShared-io.vertx.core.Vertx-io.vertx.core.json.JsonObject-) 或者 [`PostgreSQLClient.createShared`](http://vertx.io/docs/apidocs/io/vertx/ext/asyncsql/PostgreSQLClient.html#createShared-io.vertx.core.Vertx-io.vertx.core.json.JsonObject-) 时加上了具有唯一名称的数据源参数。

## 关闭客户端

您可以较长时间的持有客户端对象（比如在 Verticle 的整个生命周期里），可一旦不再使用，就应该使用[`close`](http://vertx.io/docs/apidocs/io/vertx/ext/asyncsql/AsyncSQLClient.html#close-io.vertx.core.Handler-) 或者[`close`](http://vertx.io/docs/apidocs/io/vertx/ext/asyncsql/AsyncSQLClient.html#close--)来关闭它。

## 获取数据库连接

您可以使用 [`getConnection`](http://vertx.io/docs/apidocs/io/vertx/ext/asyncsql/AsyncSQLClient.html#getConnection-io.vertx.core.Handler-) 来获得数据库连接。
